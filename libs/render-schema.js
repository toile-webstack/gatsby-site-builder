...
const isElem = level => level && typeof level === 'object' && !isArray(level)
// &&
...
const renderSchema = ({
  ...
  const renderElement = ({ component, children, css, ...props }) => {
    ...
    const Comp = determineComponent({ component, tag, children })
  ...
  const dispatchToRender = (something, isFirstIteration) => {
    ...
    if (something.component === 'children') {
    ...
    if (t === 'string') {
      // NOTE: Don't do that. If the string correspond to the name of a LibComp, then it will render de comp even if it should not.
    ...
    //   component: Fragment,
    //   children: renderArray(something),
    // })
